...
class VectorRegistros{
    ...
    public:
        ...
        vec_registros(vec_registros), size(vec_registros.size()){}
        ...
        /*Regresa el nombre origen de una IP dada. */
        string buscarNombreConIP(string ip);
...
string VectorRegistros::buscarNombreConIP(string ip){
    bool encontrado = false;
    string nombre_ip = "-";

    for (int i = 0; i < vec_registros.size(); i++){
        if( vec_registros[i].getIpOrigen() == ip){
            encontrado = true;
            nombre_ip = vec_registros[i].getNombreOrigen();
            break;
        }
    }
...
string VectorRegistros::desifrarRedInterna(){
    ...
    if ( ip_Actual[ip_Actual.size()-4] == '.' ){         //Si las ultimas son 3 cifras
        return ip_Actual.substr(0,ip_Actual.size()-4);

    } else if ( ip_Actual[ip_Actual.size()-3] == '.' ){  //Si las ultimas son 2 cifras
        return ip_Actual.substr(0,ip_Actual.size()-3);
        
    } else if ( ip_Actual[ip_Actual.size()-2] == '.' ){  //Si las ultimas es 1 cifra
        return ip_Actual.substr(0,ip_Actual.size()-2);
